...
def confusion_matrix(reslist):
    keepres = [x for x in reslist if not np.isnan(x.stat)]
    nitems = len(keepres)
    ypred = np.array([x.stat for x in keepres])
    ytrue = np.array([x.causality for x in keepres])

    pos = len([x for x in reslist if x.causality == 1])
    neg = len(reslist) - pos
    ...
    return fpr, tpr, ppv, nsel, fdr, np.array(tplist)
