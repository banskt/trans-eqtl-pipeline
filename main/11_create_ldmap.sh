...
for MDATA in ${LD_DATASETS}; do
    source ${DATALOAD}
    OUTDIR_DATA="${GENO_DIR}/SHAPEIT2_ldmap_${LDWINDOW}_${LDMIN_R2}"
    if [ ! -d ${OUTDIR_DATA} ]; then mkdir -p ${OUTDIR_DATA}; fi

    JOBDIR_DATA="${JOBSUBDIR}/SHAPEIT2_ldmap_${LDWINDOW}_${LDMIN_R2}/${DATATYPE}"
    ...
    # else
        for CHRM in ${CHRNUMS}; do
            ...
            sed -e "s|_JOB_NAME|${JOBNAME}|g;
                    ...
                    s|_OUTF_|${LDOUTFILE}|g;
                    s|_KEEP_|${LDKEEPFILE}|g;
                   " ${MASTER_BSUBDIR}/create_ldmap.slurm > ${JOBDIR_DATA}/${JOBNAME}.slurm
