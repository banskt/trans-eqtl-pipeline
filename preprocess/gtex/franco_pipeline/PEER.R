...
if (!is.null(argv$covar) && !is.na(argv$covar)) {
    covar.df <- read.table(argv$covar, sep="\t", header=TRUE, row.names=1, as.is=TRUE, check.names=F)
    covar.df <- sapply(covar.df, as.numeric)
    cat(paste0("  * including ", dim(t(covar.df))[2], " covariates", "\n"))
    invisible(PEER_setCovariates(model, as.matrix(t(covar.df))))  # samples x covariates
}
# if(!is.null(covs)) {
