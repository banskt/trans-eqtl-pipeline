...
def find_dhs_overlap(res_dict, dhs_file, outfile):
    ...
    while line:
        ...
        prev_chrm = chrm
    dhs.close()
    return header, nannot


def contains_annotated_tissue(dhsfile, epigenomelist):
    res = False
    with open(dhsfile, 'r') as infile:
        header = infile.readline()
        hdrsplit = [x.strip() for x in header.split("\t")]
        colidx = [hdrsplit.index(x) for x in epigenomelist if x in hdrsplit]
        if len(colidx) > 0:
            res = True
    return res


def read_dhsoverlap_outfile(overlapfile, epigenomelist, labeldict):
    ...
    with open(overlapfile, 'r') as infile:
        ...
        for line in infile:
            lsplit = line.split("\t")
            chrom_mark = [lsplit[idx].strip() for idx in colidx]
            uniq_marks = set([labeldict[x] for x in chrom_mark])
            for k in uniq_marks:
                tannot[k] += 1
    return tannot


def get_chromatin_overlap(res_dict, rand_dicts, dhsfile, epigenomelist, labeldict, tmpoutdir):
    tmp_overlap_file = os.path.join(tmpoutdir, 'overlapped_regions.txt')
    #ofile = open(tmp_overlap_file, 'w')
    #nannot = find_dhs_overlap(res_dict, dhsfile, ofile)
    #ofile.close()
    tannot = read_dhsoverlap_outfile(tmp_overlap_file, epigenomelist, labeldict)

    tannot_rand = [np.zeros(len(CHROMATIN_REGIONS)) for x in rand_dicts]
    for i,rdict in enumerate(rand_dicts):
        tmp_overlap_file = os.path.join(tmpoutdir, f'rand_overlapped_regions_{i+1 :02d}.txt')
        #ofile = open(tmp_overlap_file, 'w')
        #nannot = find_dhs_overlap(rdict, dhsfile, ofile)
        #ofile.close()
        tannot_rand[i] = read_dhsoverlap_outfile(tmp_overlap_file, epigenomelist, labeldict)
...
#    tissuelist = opts.tissuelist
    ...
    labeldict = get_label_dict(method)
    ...
    for tissue in tissuelist:
        ...
        if nteqtl > 0 and len(eidlist) > 0:
            ...
            if contains_annotated_tissue(dhsfile, epigenomelist):
                ...
                tannot, tannot_rand = get_chromatin_overlap(res_dict, rand_dicts, dhsfile, epigenomelist, labeldict, tmpoutdir)
                ...
                print("No annotated tissue in Segway")
