...
master_snp_data_file = "/cbscratch/franco/datasets/gtex_v8_dhs_{:s}_background_SHAPEIT2_Fst_matched_per_tissue.txt".format(title)
...
all_snp_ids = pd.read_csv(os.path.join("/cbscratch/franco/from_saikat/gtex_v8_202003/all_variants_pvalues_tejaas_4optims.txt"), usecols=[0], header=0, sep="\t")
...
for use_LD in LDs:
    for gamma_suffix in gamma_suffixes:
        for pcutoff in pcutoffs:
            ...
            outdir = os.path.join(resdir, "dhs_enrichments_fst_matched")
