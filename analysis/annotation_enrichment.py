...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Calculate annotation enrichment for trans-eQTLs')

    parser.add_argument('--annot',
                        type=str,
                        dest='annotfile',
                        metavar='FILE',
                        help='full path of the annotation file')
...
fout = open(outfile, 'w')
fout.write(f'TISSUE\tN_TRANS\tN_ANNOT\tFRAC\tENRICHMENT\tP_VALUE\n')
...
for tissue in tissuelist:
    ...
    if nteqtl > 0:
        ...
        fout.write(f'{tissue}\t{nteqtl}\t{n_annotated}\t{dhs_frac_tissue}\t{dhs_enrichment}\t{dhs_enrichment_pval}\n')

        print (f'Fraction of trans-eQTLs in annotation region: {dhs_frac_tissue}')
